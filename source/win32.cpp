...
FARPROC loadDLLs(std::string dll) {
    ...
    if (getenv("VS_ILS_VERBOSE"))
        std::wcerr << L"vs-ils: preloading " << p << L": " << h << std::endl;
    if (!h) {
        std::wcerr << L"vs-ils: failed to preload " << p << std::endl;
        h = LoadLibraryA(dll.c_str());
    }
    if (!h)
        std::cerr << "vs-ils: failed to preload " << dll << std::endl;
    return (FARPROC)h;
}
